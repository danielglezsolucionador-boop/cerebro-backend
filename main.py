...
async def agent_report(payload: dict, current_user=Depends(get_current_user)):
    ...
    try:
        ...
        level, reasoning, correlations = process_report(db, record.id, record.agent, record.message, record.category)
        return {"id": record.id, "status": "saved", "priority_level": level, "correlations": len(correlations)}
    except Exception as e:
        db.rollback()
